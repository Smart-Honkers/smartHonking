...
#ifndef __INET_PAIRNEIGHBORCARS_H
#define __INET_PAIRNEIGHBORCARS_H
 ...
 */
class INET_API pairNeighborCars : public ApplicationBase, public UdpSocket::ICallback
{
...
protected:
    ...
    void receiveMessage();
    void checkNeighorListRefresh();
    float getRadialDistance(Coord sendercord,Coord receivercord);
    float getSenderPosition(Coord sendercord,Coord receivercord);
    Coord getHostPosition(cModule *hostmodule);
    void sendResponseToSender(L3Address respDestAddress,bool pair_accepted);
    void sendPairRequest(L3Address BroadCastAddress);
    void indicateStopMessage(L3Address stopDestAddress);
